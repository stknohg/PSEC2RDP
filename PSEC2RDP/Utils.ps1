...
<#
    Parallels Client is default tool on macOS (Tested Parallels Client 19.2)
#>
...
function Get-MacOSMicrosoftRemoteDesktopScriptBlocks () {
    # Return 3 scriptblocks (begin block, main block, end block)
    return ( {
            param ([string]$HostName, [int]$Port, [PSCredential]$Credential)

            # Copy password to clipboard
            Write-Warning ('Copy {0} passwrod to clipboard.' -f $Credential.UserName)
